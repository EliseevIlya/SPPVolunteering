...
public interface CompositionRepository extends JpaRepository<Composition, Long> {
    ...
    @Query("SELECT c FROM Composition c WHERE c.user.userId = :userId AND c.event.eventId = :eventId")
    boolean findByUserIdAndEventId(@Param("userId") Long userId, @Param("eventId") Long eventId);
    @Query("DELETE FROM Composition c WHERE c.user.userId = :userId AND c.event.eventId = :eventId")
   ...
   /* @Query(value = "CREATE TEMPORARY TABLE Tempo (id BIGINT, user_id BIGINT, user_name VARCHAR(255), role_id BIGINT, role_name VARCHAR(255));", nativeQuery = true)
    ...
    @Query("SELECT c FROM Composition c WHERE c.user.userId = :userId ")
